...
const Login = () => {
  ...
  const { login, user, successMessage, clearError, clearSuccessMessage } = useAuth();

  const redirectBasedOnUserType = useCallback((user) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!loggedInUser || !loggedInUser.userType) {
        console.error('User object is missing or has no userType:', loggedInUser);
        setError('Login successful but user data is incomplete. Please contact support.');
        setIsLoading(false);
        return;
      }
